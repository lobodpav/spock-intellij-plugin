...
class NewSpockSpecificationAction : JavaCreateTemplateInPackageAction<GrTypeDefinition>(
    ...
    override fun isAvailable(dataContext: DataContext): Boolean {
        ...
        if (module.project.service<DumbService>().isDumb) {
            log.info("Spock Specification action is unavailable due to the IDE indexing")
            return false
        }

        return super.isAvailable(dataContext)
                && LibrariesUtil.hasGroovySdk(module)
                && LibrariesUtil.findJarWithClass(module, SpockUtils.SPEC_CLASS_NAME) != null
    }

    override fun getActionName(directory: PsiDirectory, newName: String, templateName: String): String = ACTION_NAME

    // Decides on where the cursor is to be placed once the Specification gets created
