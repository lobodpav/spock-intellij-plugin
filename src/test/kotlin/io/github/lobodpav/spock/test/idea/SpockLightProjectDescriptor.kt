...
import io.github.lobodpav.spock.test.idea.TestModule.ROOT
...
class SpockLightProjectDescriptor : DefaultLightProjectDescriptor() {
    ...
    override fun setUpProject(project: Project, handler: SetupHandler) {
        createAndSetupModule(project, handler, ROOT.moduleName, ROOT.sourceRootPrefix)
    ...
    private val tempDirectory: VirtualFile
        get() = VirtualFileManager.getInstance().findFileByUrl(TEMP_DIRECTORY) ?: error("Did not find the '$TEMP_DIRECTORY' directory to hold test project data")
